...
//    public static final String self_regex = "\\[([\\w\\p{InCJKUnifiedIdeographs}-]*)]\\([\\w\\p{InCJKUnifiedIdeographs}-]*\\)";
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        //适配英文版
        TEXT_CONTRACT = context.getString(R.string.view_social_contract);
        TEXT_EXPEND = context.getString(R.string.view_social_expend);
        TEXT_TARGET = context.getString(R.string.view_social_text_target);
        ...
        if (attrs != null) {
            ...
            mMentionTextColor = a.getColor(R.styleable.ExpandableTextView_ep_mention_color,
                    Color.parseColor("#FF6200"));
            int resId = a.getResourceId(R.styleable.ExpandableTextView_ep_link_res, R.mipmap.view_icon_link);
            mLinkDrawable = getResources().getDrawable(resId);
            currentLines = mLimitLines;
            a.recycle();
        } else {
            mLinkDrawable = context.getResources().getDrawable(R.mipmap.view_icon_link);
        }
